...
while (True):
    ...
    if (d<10.0):
        print(four_way.GetClosestLightState(veh.GetPosition()))
        sys.stdout.flush()

    d_north = north_four_way.GetDistanceToIntersection(veh.GetPosition())
    if (d_north<10.0):
        print(north_four_way.GetClosestLightState(veh.GetPosition()))
        sys.stdout.flush()

    d_south = south_four_way.GetDistanceToIntersection(veh.GetPosition())
    if (d_south<10.0):
        print(south_four_way.GetClosestLightState(veh.GetPosition()))
        sys.stdout.flush()
