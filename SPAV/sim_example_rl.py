...
planner = rl_planner.PathPlanner()
...
while dist_to_goal > 4.0:
    ...
    if n % 3 == 0 and n > 0:
        ...
        # calculate a path through the occupancy grid using RL
        actor_loss, critic_loss, entropy, planned_actions = train_step(
            actor, critic, optimizer_actor, optimizer_critic,
            states_batch, actions_batch, advantages_batch, returns_batch,
            entropy_weight=0.01, training=True)

        # Use planned_actions to control your simulated agent/environment
        # For example:
        for i, action in enumerate(planned_actions.numpy()):
            print(f"Agent {i} planned action: {action}")
